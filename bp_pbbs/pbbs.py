...
def view(pid):
    ...
    row = {'pid':pid, 'title':title, 'desc':desc, 'period':period, 'viewCount':view_count,
           'course_name':course_name, 'course_org':course_org, 'hash':hash_tags,
           'authors':authors, 'files':files}
    return render_template('pbbs/view.html', menu=menu, row=row)